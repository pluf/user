<?php
use Pluf\Exception;
use Pluf\User\Form\UserChangeEmailFrom;
...
class User_Views_Mail
{
    ...
    {
        $key = $match[1];
        list ($email, $id, $time) = UserChangeEmailFrom::validateKey($key);
        ...
        $request->user->update();
        $request->user->setMessage(sprintf('Your new email address "%s" has been validated. Thank you!', Pluf_esc($email)));
