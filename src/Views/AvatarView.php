 ...
 *        
 */
class AvatarView extends \Pluf\Views
{
    ...
    public static function update($request, $match)
    {
        if (array_key_exists('userId', $match)) {
            // Check user is owner
            if ($request->user->id != $match['userId'] && ! Precondition::isOwner($request)) {
                throw new PermissionDeniedException('Not allowed to change others avatar');
            }
            $user = Pluf_Shortcuts_GetObjectOr404('User_Account', $match['userId']);
        } else {
            $user = $request->user;
