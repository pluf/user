 ...
 *        
 */
class PasswordView extends \Pluf\Views
{
    ...
    public function password($request, $match)
    {
        ...
        if (array_key_exists('token', $request->REQUEST)) {
            ...
            $sql = new SQL('token=%s', array(
                $request->REQUEST['token']
            ));
            $token = $token->getOne($sql->gen());
            if (! $token || $token->isExpired()) {
                throw new DoesNotExistException('Token not exist');
            }
            $user = $token->get_user();
        ...
        if (array_key_exists('old', $request->REQUEST) || array_key_exists('old_password', $request->REQUEST)) {
            ...
            // Retreive credential
            $cred = Credential::getCredential($request->user->id);
            if (! $cred->checkPassword($oldPass)) {
                throw new MismatchParameter('Old pass is not currect');
            }
        ...
        throw new MismatchParameter('Invalid request params');
    }
    ...
    private function sendPasswordToken($request, $user)
    {
        ...
        // 3- Notify user
        if (Bootstrap::f('test_unit', false)) {
            return;
        }
