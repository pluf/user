 ...
 *        
 */
class RoleGroupView extends \Pluf\Views
{
     ...
     * @param array $match
     */
    public static function find(Request $request, $match)
    {
        $perm = new Role($match['role_id']);
        $grModel = new Group();
        $pag = new Paginator($grModel);
        $sql = new SQL('user_role_id=%s', array(
            $perm->id
        ));
     ...
     * @param array $match
     */
    public static function get(Request $request, $match)
    {
        $perm = Pluf_Shortcuts_GetObjectOr404('User_Role', $match['role_id']);
        ...
        if ($groups->count() == 0) {
            throw new DoesNotExistException('Group has not such role');
        }
        return $groups[0];
    }
