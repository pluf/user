...
class LdapBackend
{
    ...
    public static function authenticate($auth_data)
    {
        ...
        $ldap_dn = Bootstrap::f('auth_ldap_dn', 'ou=users,dc=example,dc=com');
        $ldap_user = Bootstrap::f('auth_ldap_user', null);
        $ldap_password = Bootstrap::f('auth_ldap_password', null);
        $ldap_version = Bootstrap::f('auth_ldap_version', 3);
        $ldap_user_key = Bootstrap::f('auth_ldap_user_key', 'uid');
        // If auth_ldap_password_key, it will use crypt hash control
        // to test the login password, else it will bind.
        $ldap_password_key = Bootstrap::f('auth_ldap_password_key', null);
        $ldap_surname_key = Bootstrap::f('auth_ldap_surname_key', 'sn');
        $ldap_givenname_key = Bootstrap::f('auth_ldap_givenname_key', 'cn');
        $ldap_email_key = Bootstrap::f('auth_ldap_email_key', 'mail');

        $ldap = ldap_connect(Bootstrap::f('auth_ldap_host', 'localhost'));
        ldap_set_option($ldap, LDAP_OPT_PROTOCOL_VERSION, Bootstrap::f('auth_ldap_version', 3));
        if (! ldap_bind($ldap, $ldap_user, $ldap_password)) {
            Log::warn(sprintf('Cannot bind to the ldap server, user:%s, password:***', $ldap_user));
            ldap_close($ldap);
            return false;
        }
        // Go for a search
        $search = ldap_search($ldap, $ldap_dn, '(' . $ldap_user_key . '=' . $login . ')', array(
            $ldap_user_key,
            $ldap_surname_key,
            $ldap_givenname_key,
            $ldap_email_key
