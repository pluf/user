...
class Account extends Model
{
    ...
    function preDelete()
    {
        ...
        Signal::send('User_Account::preDelete', 'User_Account', $params);
    ...
    {
        if ($this->isAnonymous()) {
            throw new DoesNotExistException("User not exist, while you are trying to add message?!");
        }
        $m = new Message();
        $m->account_id = $this;
        $m->message = $message;
        if (! $m->create()) {
            throw new Exception("not possible to create a message");
        }
        return $m;
    }
    ...
    function getProfile()
    {
        $pclass = Bootstrap::f('user_profile_class', '\Pluf\User\Profile');
        $db = $this->getDbConnection();
        $sql = new SQL(sprintf('%s=%%s', $db->qn('user')), array(
            $this->id
        ));
        $users = new $pclass();
        $users->getList(array(
        ...
        if ($users->count() != 1) {
            throw new DoesNotExistException(sprintf('No profiles available for user: %s', (string) $this));
        }
        return $users[0];
    }
